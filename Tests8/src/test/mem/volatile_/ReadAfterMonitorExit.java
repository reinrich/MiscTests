...
public class ReadAfterMonitorExit {
    ...
    // separate volatiles to different cache lines
    static long tmp11,tmp12,tmp13,tmp14,tmp15,tmp16,tmp17,tmp18;

    static volatile int spinLock;
    private static final int OWNED_BY_A = 0;
    private static final int OWNED_BY_B = 1;
    ...
    static class WriteB extends Thread {
        public void run() {
            ...
            final Object theLock = lock;  
            for(int i = 0; i < n; i++) {
                while (spinLock != OWNED_BY_B);
                synchronized(theLock) {
//                    test.Tools.killDCache(Tools.BY_WRITING);
                    test.Tools.killDCachePartially(Tools.BY_WRITING, i, 30);
                    b_nonvolatile = i;
                }
                mem[i] = a_volatile;
                spinLock = OWNED_BY_A;
    ...
    public static void main(String args[]) throws Exception {
        System.out.println("*** ");
        System.out.println("*** Running " + ReadAfterMonitorExit.class);
        System.out.println("*** ");
        System.out.println();
...
class MyLock {
    // separate lockword to different cache line
    long tmp21,tmp22,tmp23,tmp24,tmp25,tmp26,tmp27,tmp28;
}